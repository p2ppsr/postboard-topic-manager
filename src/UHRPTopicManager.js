    ...
    try {
      ...
      for (const [i, output] of parsedTransaction.outputs.entries()) {
        ...
        try {
          ...
          if (result.fields[3].toString('utf8') !== 'advertise') {
          ...
          if (contentLength <= 0) {
            const e = new Error(`Content length must be a positive value: ${contentLength}`)
          ...
          if (contentLength > 11000000000) {
            const e = new Error(`Currently, the maximum supported file size is 11000000000 bytes:${contentLength}. Larger files will be supported in future versions, but consider breaking your file into chunks. Email nanostore-limits@babbage.systems if this causes you pain.`)
